...
import BACKEND_URL from '../api'; // ✅ Centralized backend config
...
const AdminPanel = () => {
  ...
  const fetchRooms = async () => {
    ...
    try {
      const res = await axios.get(`${BACKEND_URL}/api/available-rooms?date=${today}`);
      setRooms(res.data);
    } catch (err) {
        ...
        <tbody>
          {reservations.map(r => (
            <tr key={r.Reservation_ID}>
              ...
              <td>
                <select
                  value={selectedRoom[r.Reservation_ID] || ''}
                  onChange={(e) => handleRoomChange(r.Reservation_ID, e.target.value)}
                >
